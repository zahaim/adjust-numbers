...
export function NumberAdjuster({
  ...
  const emit = useCallback(
    (nextDigits: number[], nextSepIndex: number | null) => {
  ...
  const handleDeleteDigit = (idx: number) => {
    if (digits.length <= 1) return;
    LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
    const next = digits.filter((_, i) => i !== idx);
    let nextSep = separatorIndex;
    if (nextSep !== null) {
      if (idx < nextSep) nextSep -= 1;
      if (nextSep <= 0 || nextSep >= next.length) nextSep = null;
    }
    emit(next, nextSep);
  };
  ...
  const handleGapPress = (gapIndex: number) => {
    hapticMedium();
  ...
  return (
    <View style={[styles.outer, { backgroundColor: resolvedBg }, style]}>
      {/* Left add button */}
          ...
          <React.Fragment key={idx}>
            ...
            {idx > 0 && (
              <SeparatorGap
                ...
                onPress={() => handleGapPress(idx)}
              />
            )}

            {/* Inline separator rendered between digits */}
            {separatorIndex === idx && !placementMode && (
              <Text style={[styles.inlineSep, { color: resolvedDigitColor, fontFamily: defaultTheme.fontFamily }]}>
                {separatorChar}
              </Text>
            )}
            ...
            <DigitWheel
              value={digit}
              digitColor={resolvedDigitColor}
              onIncrement={() => handleIncrement(idx)}
              onDecrement={() => handleDecrement(idx)}
              onLongPressDelete={() => handleDeleteDigit(idx)}
              canDelete={digits.length > 1}
            />
          </React.Fragment>
        ))}
      </ScrollView>
      ...
      <SeparatorControl
        ...
        onToggle={handleSeparatorToggle}
        visible={digits.length > 1}
      />
